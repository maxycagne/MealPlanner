    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        root.txtFilter.setAdapter(arrayAdapter);*/
            ...
            Executors.newSingleThreadExecutor().submit(()->{
                List<Dishes> userList = dbhelper.getDishesDao().getDishes();
                handler.post(() -> {
                    if (!userList.isEmpty()) {
                        ArrayAdapter<Dishes> arrayAdapter = new ArrayAdapter<>(getContext(),android.R.layout.simple_list_item_1, userList);
                        root.txtFilter.setAdapter(arrayAdapter);
